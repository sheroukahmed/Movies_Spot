...
class PopularViewModel {
    ...
    func loadData() {
        network?.fetch(url: APIManger.getFullURL(details: "popular") ?? "", type: EventResponse.self, completionHandler: { [weak self] result, error in
            ...
            CoreDataManager.shared.deleteAllMovies(EnityValue: "Popular")
            
            if let movies = result.results {
                for movie in movies {
                    CoreDataManager.shared.storeEvent(Event: movie, EnityValue: "Popular")
                }
            }
        })
    }
    ...
    func loadDatafromCoreData() {
        let storedMovies = CoreDataManager.shared.getEvent(EnityValue: "Popular")
